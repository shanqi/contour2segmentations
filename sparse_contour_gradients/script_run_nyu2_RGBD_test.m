...
depthdir=['./nyu_v2/data/depth/' dataset '/'];
...
if run_local
    outdir=['scg_RGBD_local_nyu2/' dataset '/'];
else
    outdir=['scg_RGBD_global_nyu2/' dataset '/'];
end
...
%for ifile=nfile-icpu+1:-ncpu:1,
  ...
  if ~exist([outdir '/' id '.mat']),
    ...
    depth=imread([depthdir '/' id '.png']);
    depth=double(depth)/1000;
    normal=compute_surface_normal(depth);
    if run_local,
      [scg_thin]=scg_pb_orient_RGBD(img,depth,normal,scg_model_RGBD);
      gPb_thin=single(scg_thin);
    else
      [scg_thin,scg_global]=scg_pb_orient_RGBD(img,depth,normal,scg_model_RGBD);
